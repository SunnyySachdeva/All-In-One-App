...
- [x] **Dark Glass Theme** - Add a new dark glass theme with fancy black/gray look to theme selector (User requested redo)
- [x] **Enhanced Dashy Tab** - Rename to 'Self hosted stuff', add '+' button for dynamic quick links with popup for link text, home icon with database persistence
- [x] **Smart Terminal** - Remove hardcoded directory, use actual current directory, handle different OS types (Linux/Windows)
- [x] **Settings Panel** - Add settings button with Ollama endpoint and OpenAI API key configuration, integrate with Videos tab summary feature
- [x] **Config Import/Export** - Add config.yml file with all settings, import/export via Settings panel
